...
if len(dataset_train_val) == 0:
    ...
    def compute_f1_macro_multi_label(logits, targets, thresholds=0.5):
        ...
        # Cálculo de precisão e recall (macro)
    ...
    def save_confusion_matrix(epoch, preds, targets, phase):
        # Reduz os rótulos multi-label para a classe mais ativada (Right Label)
        right_labels = targets.argmax(axis=1)
        pred_labels = preds.argmax(axis=1)  # Classe mais ativada como predição

        # Calcula matriz de confusão 6x6
        conf_matrix = confusion_matrix(right_labels, pred_labels, labels=range(6))

        # Criar DataFrame com formato correto
        conf_list = []
        for true_class in range(6):  # Para cada classe real
            row = [epoch, phase, CLASS_NAMES[true_class]] + conf_matrix[true_class].tolist()
            conf_list.append(row)

        # Criar cabeçalhos corretos com os nomes das doenças
        ...
        # Verifica se o arquivo já existe para anexar dados
    ...
    for epoch in tqdm(range(1, num_epochs+1), desc="Treinamento"):
        ...
        # Chamar a função para cada métrica após cada época
