...
import pandas as pd
import json
from dataloader import LazyGraphPtDataset


# ================================================
# 1. Dataset: Carregamento dos exames de forma lazy
# ================================================
# Em vez de carregar todo o arquivo (como na ECGDataset), usamos o LazyGraphPtDataset,
# que carrega os grafos sob demanda, utilizando arquivos de índice se disponíveis.
#
# Suponha que os arquivos .pt já foram salvos com os grafos e os rótulos.
#
# Para treino/validação:
train_val_file = "/scratch/guilherme.evangelista/Clustering-Paper/Grafo/dataset/codetest2.pt"
# Para teste:
test_file = "/scratch/guilherme.evangelista/Clustering-Paper/Grafo/dataset/codetest2.pt"

print("Carregando os datasets (modo lazy):")
dataset_train_val = LazyGraphPtDataset([train_val_file])
dataset_test      = LazyGraphPtDataset([test_file])

print(f"\nTotal de exames em TREINO/VAL: {len(dataset_train_val)}")
print(f"Total de exames em TESTE: {len(dataset_test)}")

# ================================================
# 2. (Opcional) Se houver múltiplos arquivos para treino/validação,
#    pode-se usar ConcatDataset. Aqui, usamos apenas um arquivo para exemplificar.
# ================================================
# Exemplo: dataset_train_val = ConcatDataset([dataset1, dataset2, ...])
...
def compute_class_distribution(dataset):
    labels = []
    # Como o sample agora é um dicionário, acessamos o rótulo em sample["label"]
    for sample in tqdm(dataset, desc="Computando distribuição de classes"):
        # Converte o rótulo para tensor (supondo que sample["label"] seja uma lista ou array)
...
if len(dataset_train_val) == 0:
    ...
    groups = []
    for sample in dataset_train_val:
        exam_id = sample["exam_id"]
        patient_id = exam_to_patient.get(exam_id)
        if patient_id is None:
            raise ValueError(f"Exam_id {exam_id} não encontrado nos metadados.")
