...
def load_usr_dataset_by_name(fname, length):
    ...
    assert path.isfile('{}/{}/{}_TEST.tsv'.format(dir_path, fname, fname)), '{} NOT EXIST in UCR:{}!'.format(fname, dir_path + '/' + fname)
    ...
    x_train, y_train = train_data.values[:, init:].astype(np.float64).reshape(-1, length, 1), \
                       train_data[0].values.astype(np.int)
    x_test, y_test = test_data.values[:, init:].astype(np.float64).reshape(-1, length, 1), \
