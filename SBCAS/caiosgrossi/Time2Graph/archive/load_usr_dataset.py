...
def load_usr_dataset_by_name(fname, length):
    ...
    x_train, y_train = train_data.values[:, init:].astype(np.float32).reshape(-1, length, 1), \
                       train_data[0].values.astype(np.int32)
    x_test, y_test = test_data.values[:, init:].astype(np.float32).reshape(-1, length, 1), \
                     test_data[0].values.astype(np.int32)
